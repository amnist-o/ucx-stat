...
## 
Assume that it is known from previous experiments that $\mu = 3$  and $\sigma = 15$. Use both, analytic and simulation-based power analysis explained in the previous two sections to evaluate the minimum sample size $n$ to achieve statistical power of at least 80% at 5% statistical significance level.

```{r load packages and define function, message=FALSE}
library(tidyverse)
...
Find the minimum sample size $n$ which gives specific statistical power at 5% significant level.
...
Plot the simulated vs actual graph of sample size and statistical power at 5% significant level
